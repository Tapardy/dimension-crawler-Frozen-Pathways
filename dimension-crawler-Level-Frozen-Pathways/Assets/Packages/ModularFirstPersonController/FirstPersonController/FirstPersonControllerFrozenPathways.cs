...
public class FirstPersonControllerFrozenPathways: MonoBehaviour
{
    private Rigidbody rb;
    public bool wasAirborne = false;
    public float jumpDistance = 2;
    public bool isJumping = false;
    public float groundedFriction = 0.6f; // Friction when grounded
    public float airFriction = 0f; // Friction when not grounded
    ...
    void FixedUpdate()
    {
        if (isGrounded)
        {
            // Set friction when grounded
            capsuleCollider.material.dynamicFriction = groundedFriction;
            capsuleCollider.material.staticFriction = groundedFriction;
        ...
        {
            // Set friction when not grounded
            capsuleCollider.material.dynamicFriction = airFriction;
            capsuleCollider.material.staticFriction = airFriction;

        }

        // Prevent midair movement during a jump
        if (!isGrounded)
        {
            return; // Exit the FixedUpdate method early if the player is not grounded
        }

        #region Movement

        if (playerCanMove && isGrounded)
        {
            // Calculate the input direction
            Vector3 inputDirection = new Vector3(Input.GetAxis("Horizontal"), 0, Input.GetAxis("Vertical"));
            ...
            // Checks if the player is walking and is grounded
            // Will allow head bob
            if (inputDirection.x != 0 || inputDirection.z != 0 && isGrounded)
            {
                isWalking = true;
            }
            else
            ...
            // Transform the input direction to match the player's orientation
            inputDirection = transform.TransformDirection(inputDirection);

            // Apply a force that attempts to reach the target velocity
            Vector3 velocity = rb.velocity;
            Vector3 velocityChange = (inputDirection - velocity);
    ...
    private void CheckGround()
    {
        ...
        float distance = jumpDistance; // Use jumpDistance as the default distance

        // Cast a raycast with layer mask to ignore the "NotGroundedGround" layer
        int layerMask = ~LayerMask.GetMask("NotGroundedGround"); // Inverts the layer mask to ignore the specified layer
        RaycastHit hit;

        if (Physics.Raycast(origin, direction, out hit, distance, layerMask))
        {
            Debug.DrawRay(origin, direction * distance, Color.red);
            wasAirborne = !isGrounded; // Set wasAirborne to true if the player was previously airborne
            isGrounded = true;
            isJumping = false;
        }
        else
        {
            isGrounded = false;
            wasAirborne = false; // Set wasAirborne to false if the player is airborne in the current frame
        }
    }
...
// Custom Editor
#if UNITY_EDITOR
[CustomEditor(typeof(FirstPersonControllerFrozenPathways)), InitializeOnLoadAttribute]
public class FirstPersonControllerEditorFrozenPathways : Editor
{
    FirstPersonControllerFrozenPathways fpc;
    SerializedObject SerFPC;

    private void OnEnable()
    {
        fpc = (FirstPersonControllerFrozenPathways)target;
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.sprintSpeed =
            EditorGUILayout.Slider(
                ...
                fpc.sprintSpeed, fpc.walkSpeed, 50f);
        fpc.jumpDistance = EditorGUILayout.Slider(new GUIContent("jump distance", "Jump distance."), fpc.jumpDistance,
            fpc.jumpDistance, 50f);
